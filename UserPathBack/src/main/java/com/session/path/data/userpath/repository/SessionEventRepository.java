 ...
 * @date 2023/02/25
 */
public interface SessionEventRepository extends JpaRepository<SessionEventEntity, Integer> {


    /**
     * 获取不去重路径结果（from to weight path）
     */
    @Transactional
    @Query(value = "select \n"
            + "t1.f_age as f_age\n"
            ...
            + ",f_user_scene_version_id\n"
            + "from \n"
            + "t_session_log_event a,(select @f_user_id \\:= null, @f_event \\:= null, "
            + "@f_session_id \\:= 0, @f_category \\:= null, @f_subcategory \\:= null, @f_rank \\:= 0, "
            + "@f_event_path = null) b \n"
            ...
            + ") m1 join\n"
            + "(select f_user_id,f_session_id,max(f_session_rank) as max_session_rank from t_session_log_event "
            + " where f_user_scene_version_id=?1 and f_make_version_id=?2\n"
            ...
            + ",GROUP_CONCAT(concat('层级',f_session_rank,'_',if(f_subcategory is null or f_subcategory='','未知',"
            + "f_subcategory)) order by  f_session_rank asc  separator '/') as f_subcategory_path\n"
            + "from \n"
            + "(\n"
            + "select \n"
            + "*\n"
    ...
    List<Map<String, Object>> getSankey(String fUserSceneVersionId,int fMakeVersionId);


    /**
     * 获取event事件不去重路径结果（进行筛选查询）
     *
     * @return
     */
    @Transactional
    @Query(value =
            "select f_category_from,f_category_to,f_subcategory_from,f_subcategory_to,f_event_from,f_event_to,"
                    ...
                    + " group by f_category_from,f_category_to,f_subcategory_from,"
    ...
    List<Map<String, Object>> getSessionEventEntity(String fUserSceneVersionId,
    ...
    List<Map<String, Object>> getSessionCategoryEntity(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom,
    ...
    List<Map<String, Object>> getSessionSubcategoryEntity(
    ...
    @Modifying
    @Transactional
    @Query(value = "select f_category as category from (select f_category,count(*) as cnt from t_user_log_orig "
            + " where f_user_scene_version_id=?1 "
            + " and f_category!='' and f_category is not null group by f_category "
            + " order by cnt desc)a", nativeQuery = true)
    List<Map<String, Object>> getSessionEventCategorySample(String fUserSceneVersionId);
    ...
    @Modifying
    @Transactional
    @Query(value = "select f_category as category from (select f_category,count(*) as cnt from t_user_log_orig_make "
            + "where f_user_scene_version_id=?1 and f_make_version_id=?2 "
            + "and f_category!='' and f_category is not null group by f_category "
            + " order by cnt desc)a", nativeQuery = true)
    List<Map<String, Object>> getSessionEventCategoryMakeSample(String fUserSceneVersionId, int fMakeVersionId);
    ...
    @Modifying
    @Transactional
    @Query(value = "select f_subcategory as subcategory from (select f_subcategory,count(*) as cnt "
            + " from t_user_log_orig where "
            + " f_user_scene_version_id=?1 "
            + " and f_subcategory!='' and f_subcategory is not null group by f_subcategory "
            + " order by cnt desc)a", nativeQuery = true)
    List<Map<String, Object>> getSessionEventSubCategorySample(String fUserSceneVersionId);
    ...
    @Transactional
    @Query(value = "select f_subcategory as subcategory from (select f_subcategory,count(*) as cnt "
            + " from t_user_log_orig_make where "
            + " f_user_scene_version_id=?1 and f_make_version_id=?2 "
            + " and f_subcategory!='' and f_subcategory is not null group by f_subcategory "
            + " order by cnt desc)a", nativeQuery = true)
    List<Map<String, Object>> getEventSubMakeSample(String fUserSceneVersionId, int fMakeVersionId);
    ...
    @Modifying
    @Transactional
    @Query(value = "select f_event as event from (select f_event,count(*) as cnt from t_user_log_orig "
            + " where f_user_scene_version_id=?1 "
            + " and f_event!='' and f_event is not null group by f_event order by cnt desc)a", nativeQuery = true)
    List<Map<String, Object>> getSessionEventSample(String fUserSceneVersionId);
    ...
    @Modifying
    @Transactional
    @Query(value = "select f_event as event from (select f_event,count(*) as cnt from t_user_log_orig_make "
            + " where f_user_scene_version_id=?1 and f_make_version_id=?2 "
            + " and f_event!='' and f_event is not null group by f_event order by cnt desc)a", nativeQuery = true)
    List<Map<String, Object>> getSessionEventMakeSample(String fUserSceneVersionId, int fMakeVersionId);
    ...
    @Query(value = "select distinct f_category_from from t_session_event_sankey "
            + " where f_user_scene_version_id=?1 and f_make_version_id=?2"
            + " order by CONVERT(substring(substring_index(f_category_from,'_',1),3,10), UNSIGNED INTEGER) asc",
            nativeQuery = true)
    List<Map<String, Object>> getSessionEventCategoryList(String fUserSceneVersionId,int fMakeVersionId);
    ...
    @Modifying
    @Transactional
    @Query(value = "select distinct f_subcategory_from from t_session_event_sankey "
            + " where f_user_scene_version_id=?1 and f_make_version_id=?2"
            + " and if(IFNULL(?3,'') !='',f_category_from like concat('%', ?3, '%'),1=1) order by "
            + " CONVERT(substring(substring_index(f_subcategory_from,'_',1),3,10), UNSIGNED INTEGER) asc",
            nativeQuery = true)
    List<Map<String, Object>> getSessionEventSubCategoryList(String fUserSceneVersionId,int fMakeVersionId,
            String fCategoryFrom);
    ...
    @Query(value = "select distinct f_event_from from t_session_event_sankey "
            + " where f_user_scene_version_id=?1 and f_make_version_id=?2"
            + " and if(IFNULL(?3,'') !='',f_category_from like concat('%', ?3, '%'),1=1) "
            + " and if(IFNULL(?4,'') !='',f_subcategory_from like concat('%', ?4, '%'),1=1) order by "
            + "CONVERT(substring(substring_index(f_event_from,'_',1),3,10), UNSIGNED INTEGER) asc", nativeQuery = true)
    List<Map<String, Object>> getSessionEventList(String fUserSceneVersionId,int fMakeVersionId, String fCategoryFrom,
    ...
    List<Map<String, Object>> getCateSankeyCntByChannelFrom(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom,
            String fCategory,
            String fCategoryFrom1,String fCategoryFrom2,String fCategoryFrom3,String fCategoryFrom4,
    ...
    @Query(value = "select f_channel,f_category_from,f_category_to,"
            ...
            + " order by category_session desc ", nativeQuery = true)
    List<Map<String, Object>> getCateSankeyCntByChannelTo(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom,
            String fCategory,
            String fCategoryFrom1,String fCategoryFrom2,String fCategoryFrom3,String fCategoryFrom4,
    ...
    List<Map<String, Object>> getSubSankeyCntByChannelFrom(String fUserSceneVersionId,int fMakeVersionId,
    ...
    @Query(value =
            "select f_channel,f_category_from,f_category_to,f_subcategory_from,f_subcategory_to,"
                    ...
                    + " order by subcategory_session desc ", nativeQuery = true)
    List<Map<String, Object>> getSubSankeyCntByChannelTo(String fUserSceneVersionId,int fMakeVersionId,
            String fCategoryFrom, String fSubcategoryFrom,
            String fCategory, String fSubcategory,
            String fCategoryFrom1,String fSubcategoryFrom1,
            String fCategoryFrom2,String fSubcategoryFrom2,
    ...
    List<Map<String, Object>> getEventSankeyCntByChannelFrom(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom,
    ...
    @Query(value =
            "select f_channel,f_category_from,f_category_to,f_subcategory_from,"
                    ...
                    + " order by event_session desc ", nativeQuery = true)
    List<Map<String, Object>> getSessionEventSankeyCntByChannelTo(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom,
            String fSubcategoryFrom, String fEventFrom, String fCategory, String fSubcategory, String fEvent,
            String fCategoryFrom1,String fSubcategoryFrom1,String fEventFrom1,
            String fCategoryFrom2,String fSubcategoryFrom2,String fEventFrom2,
    ...
    @Query(value = "select f_province,f_category_from,"
            ...
            + " order by category_session desc ", nativeQuery = true)
    List<Map<String, Object>> getCateSankeyCntByProvFrom(String fUserSceneVersionId,
    ...
    @Query(value = "select f_province,f_category_from,f_category_to,"
            ...
            + " order by category_session desc ", nativeQuery = true)
    List<Map<String, Object>> getCateSankeyCntByProvTo(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom, String fCategory,
            String fCategoryFrom1,String fCategoryFrom2,String fCategoryFrom3,String fCategoryFrom4,
            String fCategoryFromA,String fCategoryToA);
    ...
    @Query(value =
            "select f_province,f_category_from,f_subcategory_from,"
                    ...
                    + " order by subcategory_session desc ", nativeQuery = true)
    List<Map<String, Object>> getSubSankeyCntByProvFrom(
            String fUserSceneVersionId,int fMakeVersionId,
            String fCategoryFrom, String fSubcategoryFrom,
            String fCategory, String fSubcategory,
            String fCategoryFrom1,String fSubcategoryFrom1,
    ...
    @Query(value =
            "select f_province,f_category_from,f_category_to,f_subcategory_from,f_subcategory_to,"
                    ...
                    + " f_category_to,f_subcategory_from,f_subcategory_to"
                    + " order by subcategory_session desc ", nativeQuery = true)
    List<Map<String, Object>> getSubSankeyCntByProvTo(String fUserSceneVersionId,int fMakeVersionId,
            String fCategoryFrom, String fSubcategoryFrom,
            String fCategory, String fSubcategory,
            String fCategoryFrom1,String fSubcategoryFrom1,
            String fCategoryFrom2,String fSubcategoryFrom2,
    ...
    @Query(value =
            "select f_province,f_category_from,f_subcategory_from,f_event_from,"
                    ...
                    + " order by event_session desc ", nativeQuery = true)
    List<Map<String, Object>> getEventSankeyCntByProvFrom(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom,
            String fSubcategoryFrom, String fEventFrom, String fCategory, String fSubcategory, String fEvent,
            String fCategoryFrom1,String fSubcategoryFrom1,String fEventFrom1,
            String fCategoryFrom2,String fSubcategoryFrom2,String fEventFrom2,
    ...
    @Query(value =
            "select f_province,f_category_from,f_category_to,f_subcategory_from,"
                    ...
                    + " f_subcategory_from,f_subcategory_to,f_event_from,f_event_to"
                    + " order by event_session desc ", nativeQuery = true)
    List<Map<String, Object>> getEventSankeyCntByProvTo(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom,
            String fSubcategoryFrom, String fEventFrom, String fCategory, String fSubcategory, String fEvent,
            String fCategoryFrom1,String fSubcategoryFrom1,String fEventFrom1,
            String fCategoryFrom2,String fSubcategoryFrom2,String fEventFrom2,
    ...
    @Query(value = "select f_sex,f_category_from,"
            ...
            + " order by category_session desc ", nativeQuery = true)
    List<Map<String, Object>> getCateSankeyCntBySexFrom(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom,
            String fCategory,
            String fCategoryFrom1,String fCategoryFrom2,String fCategoryFrom3,String fCategoryFrom4,
            String fCategoryFromA);
    ...
    @Query(value = "select f_sex,f_category_from,f_category_to,"
            ...
            + " order by category_session desc ", nativeQuery = true)
    List<Map<String, Object>> getSessionCategorySankeyCntBySexTo(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom,
            String fCategory, String fCategoryFrom1,
            String fCategoryFrom2,String fCategoryFrom3,String fCategoryFrom4,
            String fCategoryFromA,String fCategoryToA);



    /**
     * 获取分性别小类不去重路径结果（进行筛选查询from）
     *
     * @return
     */
    @Modifying
    ...
    @Query(value =
            "select f_sex,f_category_from,f_subcategory_from,"
                    ...
                    + " order by subcategory_session desc ", nativeQuery = true)
    List<Map<String, Object>> getSubSankeyCntBySexFrom(String fUserSceneVersionId,int fMakeVersionId,
            String fCategoryFrom, String fSubcategoryFrom,
            String fCategory, String fSubcategory,
            String fCategoryFrom1,String fSubcategoryFrom1,
            String fCategoryFrom2,String fSubcategoryFrom2,
    ...
    @Query(value =
            "select f_sex,f_category_from,f_category_to,f_subcategory_from,f_subcategory_to,"
                    ...
                    + " f_category_to,f_subcategory_from,f_subcategory_to"
                    + " order by subcategory_session desc ", nativeQuery = true)
    List<Map<String, Object>> getSubSankeyCntBySexTo(String fUserSceneVersionId,int fMakeVersionId,
            String fCategoryFrom, String fSubcategoryFrom,
            String fCategory, String fSubcategory,
            String fCategoryFrom1,String fSubcategoryFrom1,
            String fCategoryFrom2,String fSubcategoryFrom2,
    ...
    @Query(value =
            "select f_sex,f_category_from,f_subcategory_from,f_event_from,"
                    ...
                    + " order by event_session desc ", nativeQuery = true)
    List<Map<String, Object>> getSessionEventSankeyCntBySexFrom(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom,
            String fSubcategoryFrom, String fEventFrom,
            String fCategory, String fSubcategory, String fEvent,
            String fCategoryFrom1,String fSubcategoryFrom1,String fEventFrom1,
            String fCategoryFrom2,String fSubcategoryFrom2,String fEventFrom2,
            String fCategoryFrom3,String fSubcategoryFrom3,String fEventFrom3,
            String fCategoryFrom4,String fSubcategoryFrom4,String fEventFrom4,
    ...
    @Query(value =
            "select f_sex,f_category_from,f_category_to,f_subcategory_from,"
                    ...
                    + " f_subcategory_from,f_subcategory_to,f_event_from,f_event_to"
                    + " order by event_session desc ", nativeQuery = true)
    List<Map<String, Object>> getSessionEventSankeyCntBySexTo(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom,
            String fSubcategoryFrom, String fEventFrom,
            String fCategory, String fSubcategory, String fEvent,
            String fCategoryFrom1,String fSubcategoryFrom1,String fEventFrom1,
    ...
    @Query(value = "select "
            ...
            + " order by category_session desc ", nativeQuery = true)
    List<Map<String, Object>> getCateSankeyCntByAgeFrom(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom, String fCategory,
            String fCategoryFrom1,String fCategoryFrom2,
            String fCategoryFrom3,String fCategoryFrom4,
            String fCategoryFromA);
    ...
    @Query(value = "select "
            ...
            + " order by category_session desc ", nativeQuery = true)
    List<Map<String, Object>> getSessionCategorySankeyCntByAgeTo(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom, String fCategory,
            String fCategoryFrom1,String fCategoryFrom2,
            String fCategoryFrom3,String fCategoryFrom4,
            String fCategoryFromA,String fCategoryToA);
    ...
    @Query(value =
            "select "
                    ...
                    + " order by subcategory_session desc ", nativeQuery = true)
    List<Map<String, Object>> getSubSankeyCntByAgeFrom(String fUserSceneVersionId,int fMakeVersionId,
            String fCategoryFrom, String fSubcategoryFrom,
            String fCategory, String fSubcategory,
            String fCategoryFrom1,String fSubcategoryFrom1,
            String fCategoryFrom2,String fSubcategoryFrom2,
    ...
    @Query(value =
            "select "
                    ...
                    + " f_category_to,f_subcategory_from,f_subcategory_to"
                    + " order by subcategory_session desc ", nativeQuery = true)
    List<Map<String, Object>> getSubSankeyCntByAgeTo(String fUserSceneVersionId,int fMakeVersionId,
            String fCategoryFrom, String fSubcategoryFrom,
            String fCategory, String fSubcategory,
            String fCategoryFrom1,String fSubcategoryFrom1,
            String fCategoryFrom2,String fSubcategoryFrom2,
    ...
    @Query(value =
            "select "
                    ...
                    + " order by event_session desc ", nativeQuery = true)
    List<Map<String, Object>> getSessionEventSankeyCntByAgeFrom(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom,
            String fSubcategoryFrom, String fEventFrom, String fCategory, String fSubcategory, String fEvent,
            String fCategoryFrom1,String fSubcategoryFrom1,String fEventFrom1,
            String fCategoryFrom2,String fSubcategoryFrom2,String fEventFrom2,
    ...
    @Query(value =
            "select "
                    ...
                    + " f_subcategory_from,f_subcategory_to,f_event_from,f_event_to"
                    + " order by event_session desc ", nativeQuery = true)
    List<Map<String, Object>> getSessionEventSankeyCntByAgeTo(String fUserSceneVersionId,
            int fMakeVersionId, String fCategoryFrom,
            String fSubcategoryFrom, String fEventFrom,
            String fCategory, String fSubcategory, String fEvent,
            String fCategoryFrom1,String fSubcategoryFrom1,String fEventFrom1,
